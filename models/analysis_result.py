...
class SecretaryDecision(BaseModel):
    ...
    followup_score: int = Field(default=0, description="明显续聊 AI 的本地评分")
    ...
    thread_topic: str = Field(default="", description="当前线程窗口的话题提示")
    addressing_mode: str = Field(default="unclear", description="当前对话主要在对谁说")
    reply_value: str = Field(default="low", description="AI 介入价值高低")
    hard_allow: bool = Field(default=False, description="是否命中本地硬放行")
    hard_suppress: bool = Field(default=False, description="是否命中本地硬压制")
    final_reason: str = Field(default="", description="最终回复/不回复的本地原因")
